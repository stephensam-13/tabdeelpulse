...
const EditUserModal: React.FC<EditUserModalProps> = ({ isOpen, onClose, user, onUpdateUser, roles }) => {
  ...
  const userRoleExists = roles.some(r => r.id === user.roleId);
              ...
              <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                <div className="flex justify-between items-center">
                    <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">
                        Edit User: <span className="font-bold text-primary">{user.name}</span>
                    </h3>
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <div>
                    ...
                    >
                      {noRolesAvailable && <option>No roles available</option>}
                      {!userRoleExists && <option value={user.roleId} disabled>Invalid Role ({user.role})</option>}
                      {roles.map(r => (
                        <option key={r.id} value={r.id}>{r.name}</option>
                      ))}
                    </select>
                    {roleDescription && (
                        <p className={`mt-2 text-sm ${!userRoleExists ? 'text-red-600' : 'text-gray-500 dark:text-gray-400'}`}>
                            {roleDescription}
                        </p>
                    )}
                  </div>
